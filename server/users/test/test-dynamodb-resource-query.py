...
def create_dynamodb_resource(aws_profile, region="ap-southeast-1"):
...
def main():
    # Create the DynamoDB Resource with the region you want
    dynamodb = create_dynamodb_resource('aws1-vlim', "ap-southeast-1")
    print("#" * 100)
    print("# list table:")
    for table in list(dynamodb.tables.all()):
        print(f"    {table}")

    table = dynamodb.Table('vlim-ws-chat-dev-user-messages-dynamodb')
    print("#" * 100)
    print("healthCheck on table")
    ...
    room_id = "nak-tino"
    try:
        response = table.query(KeyConditionExpression=Key('roomId').eq(room_id))
        ...
        for item in items:
            ...
            table.update_item(
                ...
                ExpressionAttributeValues={
                    ':val1': re.sub('"text":\s".*:\s', '"text": "', content)
                }
            )
        # Handle response
    except ClientError as error:
